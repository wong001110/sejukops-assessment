-- Phase 6 corrective migration: replace the routing RPC after the live
-- PostgreSQL environment reported that pg_catalog.jsonb_object_length(jsonb)
-- is unavailable. Count the set returned by jsonb_object_keys instead.
...
comment on function public.admin_update_ai_routing(
  uuid, public.ai_routing_mode, uuid, jsonb
) is 'Atomically replaces AI routing using a portable JSON key-count aggregate.';
