...
export function sanitizeAIProviderPayload(value: unknown, key = "", depth = 0): unknown {
  ...
  if (Array.isArray(value)) {
    const items: unknown[] = value.slice(0, MAX_ARRAY).map((item) => sanitizeAIProviderPayload(item, key, depth + 1));
  ...
  if (typeof value === "object") {
    ...
    for (const [childKey, childValue] of Object.entries(object).slice(0, MAX_KEYS)) {
      result[childKey] = sanitizeAIProviderPayload(childValue, childKey, depth + 1);
    }
...
function safeResponseHeaders(headers: Headers): Readonly<Record<string, string>> {
  const allowed = ["content-type", "x-request-id", "openai-request-id", "x-ratelimit-limit-requests", "x-ratelimit-remaining-requests", "x-ratelimit-reset-requests"];
...
export function recordAIProviderExchange(input: Omit<AIProviderExchange, "id" | "appTraceId" | "sequence" | "task" | "createdAt">) {
...
export async function runWithAIProviderObservation<T>(
  ...
  | Readonly<{ ok: true; value: T; exchanges: readonly AIProviderExchange[] }>
  | Readonly<{ ok: false; error: unknown; exchanges: readonly AIProviderExchange[] }>
> {
  const enabled = request.headers.get("x-sejuk-observe-ai") === "1";
  if (!enabled) {
    try { return { ok: true, value: await action(), exchanges: [] } as const; }
    catch (error) { return { ok: false, error, exchanges: [] } as const; }
  }
  ...
  return storage.run(context, async () => {
    try {
      return { ok: true, value: await action(), exchanges: context.exchanges } as const;
    } catch (error) {
      return { ok: false, error, exchanges: context.exchanges } as const;
    }
  });
}
