...
import { AppstoreOutlined, DashboardOutlined, FileTextOutlined, SafetyCertificateOutlined } from "@ant-design/icons";
...
export function DesktopShell({ role, identityId, children }: { role: "ADMIN" | "MANAGER"; identityId: string; children: ReactNode }) {
  const items = role === "ADMIN"
    ? [{ key: "overview", icon: <AppstoreOutlined />, label: "Operations" }, { key: "orders", icon: <FileTextOutlined />, label: "Orders (coming soon)" }, { key: "settings", icon: <SafetyCertificateOutlined />, label: "AI settings (coming soon)" }]
