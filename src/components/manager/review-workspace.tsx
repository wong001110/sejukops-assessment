...
/** A native POST form keeps the external WhatsApp launch directly user-initiated. */
function WhatsAppOpenForm({ orderId, requestKey: stableRequestKey, label = "Open WhatsApp again", disabled = false }: { orderId: string; requestKey: string; label?: string; disabled?: boolean }) {
  return <form method="post" action={`/api/manager/reviews/${orderId}/whatsapp/open`} target="_blank" className="whatsapp-open-form">
    <input type="hidden" name="requestKey" value={stableRequestKey} />
    <Button htmlType="submit" icon={<MessageOutlined />} disabled={disabled}>{label}</Button>
  </form>;
}
...
export function ReviewWorkspace() {
  ...
  const [filters, setFilters] = useState<ManagerReviewListQuery>({});
  ...
  const load = useCallback(async (nextFilters: ManagerReviewListQuery = {}) => {
    setLoading(true);
    setError(undefined);
    try {
      setQueue(await managerReviewApi.list(nextFilters));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Completed jobs could not be loaded.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const applyFilters = (next: ManagerReviewListQuery) => {
    setFilters(next);
    void load(next);
  };
  ...
  const openDetail = async (item: ManagerReviewListItem) => {
    ...
    setDetail(undefined);
    try {
      const response = await managerReviewApi.detail(item.id);
      setDetail(response.review);
    } catch (cause) {
      setFeedback({ type: "error", message: cause instanceof Error ? cause.message : "Review detail could not be loaded." });
    } finally {
  ...
  const refreshDetail = async (orderId: string) => {
    const response = await managerReviewApi.detail(orderId);
    setDetail(response.review);
  };

  const closeDecision = () => {
    if (decision) decisionKeys.current.delete(decision.review.id);
    setDecision(undefined);
    setDecisionNote("");
  };
  const submitDecision = async () => {
    if (!decision || (decision.decision === "REQUEST_CLARIFICATION" && !decisionNote.trim())) return;
    const id = decision.review.id;
    setDecisionSaving(true);
    try {
      const key = decisionKeys.current.get(id) ?? requestKey();
      decisionKeys.current.set(id, key);
      const input = decision.decision === "REQUEST_CLARIFICATION"
        ? { decision: "REQUEST_CLARIFICATION" as const, note: decisionNote.trim(), requestKey: key }
        : { decision: "APPROVE" as const, note: decisionNote.trim() || undefined, requestKey: key };
      const result = await managerReviewApi.decide(id, input);
      ...
      setFeedback({ type: "error", message: cause instanceof Error ? cause.message : "The review decision could not be saved. Retry safely with the same request." });
    } finally {
  ...
  const closeReschedule = () => {
    if (reschedule) rescheduleKeys.current.delete(reschedule.id);
    setReschedule(undefined);
    setScheduleAt("");
    setRescheduleReason("");
  };
  const submitReschedule = async () => {
    if (!reschedule || !scheduleAt) return;
    setRescheduleSaving(true);
    try {
      const key = rescheduleKeys.current.get(reschedule.id) ?? requestKey();
      rescheduleKeys.current.set(reschedule.id, key);
      ...
      rescheduleKeys.current.delete(reschedule.id);
  ...
  const closeRequest = () => {
    if (request) resolveKeys.current.delete(request.id);
    setRequest(undefined);
    setRequestSchedule("");
    setResolutionNote("");
  };
  const openRequest = (next: ManagerRescheduleRequest) => {
    resolveKeys.current.set(next.id, requestKey());
    setRequest(next);
    setRequestSchedule(next.requestedSchedule ? toMalaysiaDateTimeLocal(next.requestedSchedule) : "");
    setResolutionNote("");
  };
  const whatsappRequestKey = (review: ManagerReviewDetail) => {
    const scope = review.notification?.id ?? review.id;
    const existing = whatsappKeys.current.get(scope);
    if (existing) return existing;
    const next = requestKey();
    whatsappKeys.current.set(scope, next);
    return next;
  };
  const resolveRequest = async (decisionValue: "APPROVE" | "REJECT") => {
    if (!request || (decisionValue === "APPROVE" && !requestSchedule)) return;
    setRequestSaving(true);
    try {
      const key = resolveKeys.current.get(request.id) ?? requestKey();
      resolveKeys.current.set(request.id, key);
      const result = await managerReviewApi.resolveRequest(request.id, {
        decision: decisionValue,
        resolutionNote: resolutionNote.trim() || undefined,
        newSchedule: decisionValue === "APPROVE" ? malaysiaDateTimeLocalToIso(requestSchedule) : undefined,
        requestKey: key,
      });
      resolveKeys.current.delete(request.id);
  ...
  const columns = useMemo<ColumnsType<ManagerReviewListItem>>(() => [
    ...
    { title: "Service", dataIndex: "serviceType", width: 132 },
    { title: "Branch", key: "branch", width: 145, render: (_, item) => `${item.branch.code} · ${item.branch.name}` },
    { title: "Completed", dataIndex: "completedAt", width: 178, render: (value) => formatMalaysiaDateTime(value) },
    { title: "Final amount", dataIndex: "finalAmount", width: 125, align: "right", render: (value) => <strong>{money(value)}</strong> },
    { title: "Flags", dataIndex: "openFlagCount", width: 82, align: "center", render: (count) => count ? <Tag color="orange">{count}</Tag> : <Typography.Text type="secondary">—</Typography.Text> },
    { title: "WhatsApp", dataIndex: "notificationStatus", width: 142, render: (value) => <NotificationTag status={value} /> },
...
function ReviewDetail({ review, onRequest, onDecision }: { review: ManagerReviewDetail; onRequest: (request: ManagerRescheduleRequest) => void; onDecision: (decision: "APPROVE" | "REQUEST_CLARIFICATION") => void }) {
  const detailItems = [
    { key: "customer", label: "Customer", children: <span>{review.customerName}<br /><Typography.Text type="secondary">{review.customerPhone}</Typography.Text></span> },
    { key: "address", label: "Service address", children: review.customerAddress },
    { key: "service", label: "Service", children: review.serviceType },
    { key: "technician", label: "Technician", children: review.technician?.name ?? "Not assigned" },
    { key: "schedule", label: "Scheduled", children: <Schedule value={review.scheduledAt} /> },
    { key: "completed", label: "Completed", children: formatMalaysiaDateTime(review.completedAt) },
    { key: "quoted", label: "Quoted price", children: money(review.quotedPrice) },
    { key: "extra", label: "Extra charges", children: money(review.extraCharges) },
    { key: "final", label: "Final amount", children: <strong>{money(review.finalAmount)}</strong> },
