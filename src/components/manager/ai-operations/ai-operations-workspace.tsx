...
function SupportedQuestionWelcome({ disabled, chooseExample }: { disabled: boolean; chooseExample: (example: string) => void }) {
  return <div className="ai-conversation-empty" aria-label="Supported operations questions">
    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Start with a supported operations question" />
    <section className="ai-conversation-welcome">
      <Space align="start" size={10}>
        ...
        <div>
          ...
          <Typography.Paragraph>Completed jobs, technician performance, workload, service activity, and operational totals across supported date periods. Requests outside this scope are declined rather than guessed.</Typography.Paragraph>
        </div>
      </Space>
      <Space wrap>{supportedExamples.map((example) => <Button key={example} size="small" onClick={() => chooseExample(example)} disabled={disabled}>{example}</Button>)}</Space>
    </section>
  </div>;
}
...
export function AIOperationsWorkspace() {
  ...
  return <main className="ai-operations-page">
    ...
    <Card className="ai-operations-chat" variant="borderless" title="Conversation" extra={<Typography.Text type="secondary">Session-only context · clearable</Typography.Text>}>
      {turns.length === 0 ? <SupportedQuestionWelcome disabled={pending} chooseExample={setDraft} /> : <List className="ai-conversation-list" split={false} dataSource={turns} renderItem={(turn) => <List.Item key={turn.id}><div className="ai-conversation-turn"><div className="ai-manager-question"><Typography.Text strong>Manager</Typography.Text><Typography.Paragraph>{turn.question}</Typography.Paragraph></div><div className="ai-assistant-turn"><Typography.Text strong>SejukOps AI</Typography.Text><AssistantTurn turn={turn} retry={() => void runTurn(turn, true)} retryDisabled={pending} /></div></div></List.Item>} />}
