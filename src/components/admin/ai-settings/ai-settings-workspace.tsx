...
function SingleModelRouting({ providers, value, onChange, disabled }: { providers: readonly SafeProfile[]; value: string | null; onChange: (value: string | null) => void; disabled: boolean }) { return <div className="ai-routing-select"><Typography.Text strong>Default AI model</Typography.Text><Select showSearch optionFilterProp="label" placeholder="Not configured" options={providerOptions(providers)} value={value} onChange={onChange} allowClear disabled={disabled} /></div>; }
function TaskRouting({ providers, routes, onChange, disabled }: { providers: readonly SafeProfile[]; routes: Record<AITaskType, string | null>; onChange: (task: AITaskType, value: string | null) => void; disabled: boolean }) { return <div className="ai-task-routing">{AI_TASKS.map((task) => <div className="ai-task-route" key={task}><div><Typography.Text strong>{taskLabels[task]}</Typography.Text><Typography.Text type="secondary">{task === "DOCUMENT_UNDERSTANDING" ? "Image/scanned input requires Vision and structured output." : "Uses the explicitly selected provider only."}</Typography.Text></div><Select showSearch optionFilterProp="label" placeholder="Not configured" options={providerOptions(providers)} value={routes[task]} onChange={(value) => onChange(task, value)} allowClear disabled={disabled} /></div>)}</div>; }

function CompatibilityMatrix({ providers, mode, defaultId, routes }: { providers: readonly SafeProfile[]; mode: RoutingMode; defaultId: string | null; routes: Record<AITaskType, string | null> }) {
  ...
  const imageMissing = documentProfile ? missingImageDocumentCapabilities(documentProfile) : ["vision"] as const;
  const imageReady = Boolean(documentProfile && !imageMissing.length);
  return <div className="ai-compatibility"><Typography.Title level={5}>Compatibility</Typography.Title><List size="small" dataSource={[...AI_TASKS]} renderItem={(task) => { const selectedId = mode === "SINGLE_MODEL" ? defaultId : routes[task]; const profile = byId.get(selectedId ?? ""); const missing = profile ? missingCapabilities(profile, task) : []; const compatible = profile ? isCompatible(profile, task) : false; const description = profile ? `${profile.name} · ${profile.model}${missing.length ? ` · Missing ${missing.map((key) => capabilityLabels[key]).join(", ")}` : ""}` : "No saved provider selected"; return <List.Item className={compatible ? "is-compatible" : !profile ? "is-unconfigured" : "is-incompatible"} extra={<Tag color={compatible ? "success" : !profile ? "default" : "warning"}>{compatible ? "Compatible" : !profile ? "Not configured" : "Capability mismatch"}</Tag>}><List.Item.Meta avatar={compatible ? <CheckCircleOutlined /> : <CloseCircleOutlined />} title={taskLabels[task]} description={description} /></List.Item>; }} /><List.Item className={imageReady ? "is-compatible" : "is-incompatible"} extra={<Tag color={imageReady ? "success" : "warning"}>{imageReady ? "Image ready" : "Vision required"}</Tag>}><List.Item.Meta avatar={imageReady ? <CheckCircleOutlined /> : <CloseCircleOutlined />} title="Image / scanned Document Understanding" description={imageReady ? "Text and image/scanned documents are compatible with the selected route." : "Image/scanned inputs are blocked until a saved Vision provider is selected."} /></List.Item></div>;
}
