...
import type { TechnicianPaginationMeta } from "@/domain/technician-jobs/contracts";
...
function mergeJobs(current: TechnicianJob[], next: TechnicianJob[]) { const map = new Map(current.map((job) => [job.id, job])); for (const job of next) map.set(job.id, job); return [...map.values()]; }

function JobStatus({ status }: { status: TechnicianJob["status"] }) { return <Tag color={statusTone[status]} fill="outline">{status === "IN_PROGRESS" ? "In progress" : "Assigned"}</Tag>; }
function Schedule({ value }: { value: string | null }) { return value ? <span>{formatMalaysiaDateTime(value)}</span> : <span className="tech-muted">Schedule to be confirmed</span>; }
...
export function JobWorkspace() {
  ...
  const loadPage = useCallback(async (page: number, replace = false) => {
    ...
    try {
      const result = await technicianJobListApi.active(page, 10);
      const next = result.jobs as TechnicianJob[];
      setJobs((current) => replace ? next : mergeJobs(current, next));
  ...
  const loadJobs = useCallback(() => loadPage(1, true), [loadPage]);
  useEffect(() => { void loadJobs(); }, [loadJobs]);
  useEffect(() => {
    const node = sentinelRef.current;
    if (!node || selected || detailLoading || loading || loadingMore || error || !pagination.hasMore) return;
    const observer = new IntersectionObserver((entries) => {
      if (entries[0]?.isIntersecting) void loadPage(pagination.page + 1);
    }, { rootMargin: "240px 0px" });
    observer.observe(node);
    return () => observer.disconnect();
  }, [detailLoading, error, loadPage, loading, loadingMore, pagination.hasMore, pagination.page, selected]);

  const openJob = async (job: TechnicianJob) => { setDetailLoading(true); try { setSelected(await technicianJobApi.detail(job.id)); } catch (cause) { setFeedback({ kind: "error", message: cause instanceof Error ? cause.message : "Job detail could not be loaded." }); } finally { setDetailLoading(false); } };
  const startJob = async () => {
    if (!selected || selected.job.status !== "ASSIGNED") return;
    setStarting(true);
    try {
      const key = startKeys.current.get(selected.job.id) ?? requestKey(); startKeys.current.set(selected.job.id, key);
      const result = await technicianJobApi.start(selected.job.id, { requestKey: key }); startKeys.current.delete(selected.job.id);
  ...
  const submitReschedule = async () => {
    if (!selected || !reason.trim()) return; setRequesting(true);
    try {
      const key = rescheduleKeys.current.get(selected.job.id) ?? requestKey(); rescheduleKeys.current.set(selected.job.id, key);
      ...
      rescheduleKeys.current.delete(selected.job.id); setSelected((current) => current ? { ...current, rescheduleRequests: [result.request, ...current.rescheduleRequests] } : current); closeReschedule(); setFeedback({ kind: "success", message: "Reschedule request sent for review." });
  ...
  const completeJob = async (values: CompletionValues) => {
    if (!selected) return;
    const key = completionKeys.current.get(selected.job.id) ?? requestKey(); completionKeys.current.set(selected.job.id, key);
    ...
    await invalidateManagerDashboard(queryClient); completionKeys.current.delete(selected.job.id); setCompletionResult(result); setCompletionMode(false);
  ...
  if (completionResult) {
    const whatsappRequestKey = whatsappOpenKeys.current.get(completionResult.job.id) ?? requestKey(); whatsappOpenKeys.current.set(completionResult.job.id, whatsappRequestKey);
  ...
  return <Space direction="vertical" block className="tech-stack">
    ...
    {loading ? <LoadingJobs /> : !error && (jobs.length ? <><section className="tech-job-list" aria-label="Assigned jobs">{jobs.map((job) => <JobCard key={job.id} job={job} onOpen={() => void openJob(job)} />)}</section><div ref={sentinelRef} className="tech-infinite-sentinel" aria-live="polite">{loadingMore ? <><DotLoading color="primary" /> Loading more jobs…</> : pagination.hasMore ? "Scroll for more jobs" : `All ${pagination.total} active job${pagination.total === 1 ? "" : "s"} loaded`}</div></> : <Card className="tech-empty-card"><Empty description="No assigned jobs right now" /><p className="tech-muted tech-center">New assigned work will appear here automatically when you refresh.</p><Button fill="none" size="small" onClick={() => void loadJobs()}>Refresh jobs</Button></Card>)}
  </Space>;
}
